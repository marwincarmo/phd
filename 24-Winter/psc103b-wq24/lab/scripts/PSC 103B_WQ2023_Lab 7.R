...
# Today, we will be learning about logistic regression
# which is when you want to fit a regression model 
# to some binary data, which only takes on values of 0 or 1
# Remember that oftentimes, 1 corresponds to a "success" of some
...
# if the quality variable is good, give the new column a value
# of 1; otherwise, if it's bad, give it a value of 0

head(wine) # did this work?
...
# Just like in linear regression, we can access information
# about the model using the summary() function

summary(simple_logreg)

# Let's write out this model and interpret it
# log-odds(quality) = -10.76 + 1.06*Alcohol
...
# we would have:
# odds(wine being good) = exp(-10.76 + 1.06*Alcohol)
# or (using the rules of exponents)
# odds(wine being good) = exp(-10.76) * exp(1.06*Alcohol)
...
# This is almost a 300% increase in the odds - wow!
...
# regression is the predicted probabbility - for a particular
...
logodds # the expected log-odds are 4.08, which again, are
# hard for us to interpret
# so let's transform those into odds
...
# Finally, we can transform these odds into a probability
# using p = o/(1 + o)

prob = odds / (1 + odds)
prob
# A cab has almost 100% (98.34%) chance of being rated
# good according to our model 
...
# log-odds(wine rated good) = -12.32 + 1.04*Alcohol + 2.67*Sulphates
# Intercept: A wine with no alcohol content and no sulphates has a
# log-odds of -12.32
# Slope of Alcohol: For each 1-unit increase in alcohol content,
# holding amount of sulphates constant, the log-odds of a wine
# being rated good increase by 1.03
# Slope of Sulphates: For each 1-unit increase in sulphates,
# holding alcohol content constant, the log-odds increase
# by 2.67

# But let's interpret these in terms of the odds

# Intercept: The expected odds of a wine with no alcohol content
# and no sulphates is exp(-12.32) = .000004 (again, almost 0!)
...
# (The odds are multiplied by 2.83 for each 1-unit increase
# in alcohol content, holding the amount of sulphates constant)
...
# (The odds are multiplied by 14.44 for each 1-unit increase
# in sulphate content, holding the amount of alcohol constant)
