...
The diagonal elements of the variance-covariance matrix are the variances of the variables
Why? Because the way that a variable relates to itself is just its spread; you can use the covariance equation to see how it becomes the variance formula when the variable is the same.

The off-diagonal elements are the covariances between each pair of variables
Notice that this matrix is symmetric. The covariance between, say, temperature and ice cream sales is the same as the covariance of ice cream sales and temperature.
...
outliers can change the value of our correlation coefficient.
...
We could also create a confidence interval around our estimate of the slope using the information provided
Recall that the formula for a CI is:

$$
\mathrm{CI}_{95\%} = \hat{b_1} \pm t_{crit} \times \mathrm{SE}
$$
Estimate $\pm t_{crit} \times \mathrm{SE}$ where $t_{crit}$ is the critical value that cuts off the upper 5% of a t-distribution with N - 2 df:
...
divided by total sum of squares: $\frac{SS{reg}}{SS_{total}}$
OR: 1 - (SSresid / SStotal) $1 - (SS{resid} / SS_{total})$
