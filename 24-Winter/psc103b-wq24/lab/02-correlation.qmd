...
Right, we can only tell that the relation between the two is **positive** -- that is, the higher the temperature, the more ice cream is sold. But is this relation really strong?  That's not something we can get from the covariance, and is why we turn to correlation instead!
...
and ice cream sales, *r*(10) = 0.96, *p* < .001. ]{color=#b22222}
...
Estimate $\pm \ t_{crit} \times \mathrm{SE}$ where $t_{crit}$ is the critical value that cuts off the upper 2.5% of a t-distribution with N - 2 df:
