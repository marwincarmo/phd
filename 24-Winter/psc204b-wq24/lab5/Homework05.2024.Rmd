...
line_data <- data.frame(
  fare_range = seq(from = from = min(titanic_dat$Fare, na.rm = T) - 2,
...
titanic_dat |> 
  ...
  geom_line(data = line.data, aes(x = FNE.range, y = prob)) +
...
-   **Accuracy Rate of Null Model**: WRITE ANSWER HERE
-   **Accuracy Rate of Model 1**: WRITE ANSWER HERE
...
-   **Probability of Rose Surviving**: WRITE ANSWER HERE
-   **Probability of Jack Surviving**: WRITE ANSWER HERE
...
-   **Is the Statement Correct:** WRITE YES OR NO

-   **Why:** WRITE EXPLANATION HERE
