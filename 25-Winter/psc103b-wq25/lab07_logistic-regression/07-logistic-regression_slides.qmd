...
Fit a logistic regression model with `alcohol` as the only predictor. Use the `glm()` function, that, for the most part, is similar to the `lm()`: `outcome var ~ independent var(s)`. You have to specify a family, using `family = "binomial"` (this tells `R` that the outcome variable is binary, and to use logistic regression).
:::
...
We can mean-center our predictor variables to improve the interpretation of our intercept
...
summary(logreg_centered)$coefficients
```
...
:::fragment
:::{.callout-caution title="Exercise"}
Find the expected log-odds of a wine with 14% of alcohol content. Save he results to `odds`. You will need to find the log-odds first!
:::
:::
...
- We can transform these odds into a probability using, $p = \frac{O}{1 + O}$
...
multiple_logreg <- glm(quality_binary ~ alcohol + sulphates,
                      data = wine,
                      family = "binomial")
summary(multiple_logreg)$coefficients
```
:::
...
```{r}
interact_logreg <- glm(quality_binary ~ alcohol + sulphates + alcohol*sulphates,
                      data = wine, family = "binomial")
summary(interact_logreg)$coefficients
```

This positive interaction suggests that while alcohol and sulphates alone may not strongly increase quality, their combination does.
...
:::{.callout-caution title="Exercise"}
What is the interpretation of the intercept and the two slopes in the model?
:::
