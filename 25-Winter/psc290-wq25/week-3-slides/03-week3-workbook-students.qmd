...
p1 <- gsoep %>%
  ...
  ggplot(aes(x = "", y = prop, fill = marital)) + 
    geom_bar(stat = "identity", width = 1, color = "white") + 
    geom_text(
      aes(y = ypos, label = paste0(marital, "\n", round(prop,1), "%"))
      , color = "black"
      , size=4,
      nudge_x = .15
      ) +
  scale_fill_manual(values = cbsafe_pal$hex[2:8]) +
    coord_polar("y", start = 0) + 
    theme_void()+
...
gsoep %>%
  ...
  scale_fill_manual(values = cbsafe_pal$hex[c(2,4,5,7,8)]) +
    coord_polar("y", start = 0) + 
    theme_void()+
  theme(legend.position = "none",
        plot.title = element_text(face = "bold", hjust = 0.5, size = rel(1.4))) +
  labs(
    title = "Proportion of marital status"
  )
```
