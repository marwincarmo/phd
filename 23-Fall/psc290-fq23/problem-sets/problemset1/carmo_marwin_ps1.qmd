...
The first line of the output shows the current working directory of the R process. It was given by the function `getwd()`. The second output line says which files are in the current working directory, and was produced by the function `list.files()`. At the moment there are only two, this .qmd file and a shortcut to the problem set 1 project.
