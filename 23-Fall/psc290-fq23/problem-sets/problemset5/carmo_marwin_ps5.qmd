...
```{r load codebook}
codebook <- readxl::read_xlsx("../problemset4/codebook.xlsx")
```

And create reference data frames (if applicable)

```{r create reference dfs}
key <- readxl::read_excel(path = "../problemset4/codebook.xlsx", sheet = "Key")
...
```{r load data}
data <- readr::read_csv("../problemset4/act_subset.csv")
```
...
data_z <- data |> 
  dplyr::mutate(
    dplyr::across(c(isi_total, hads_anxiety, hads_depression),
                  z_score)
  )
```
...
```{r z-desc}
data_z |> 
  dplyr::select(isi_total, hads_anxiety, hads_depression) |> 
  tidyr::pivot_longer(cols = everything(), values_to = "z_score", names_to = "var") |> 
  dplyr::with_groups(var, summarise,
                     mean = mean(z_score, na.rm=TRUE),
                     sd = sd(z_score, na.rm=TRUE))
```

We can confirm the function worked by observing the mean of approximately zero and standard deviation of 1.
...
1. Write `both_na()`, a summary function that takes two vectors of the same length and returns the number of positions that have an `NA` in both vectors. Use it to test whether any rows in two columns of your choosing from your data are both `NA`. 
...
both_na(data$dbas16_1, data$hads_1)
```
...
- The function `f1` checks whether the prefix of a string matches the given prefix. It could be called `prefix_match()`.

- The function `f3` repeats a value the specified number of times, given by the lenght of the vector in `x`. A better name for `f3` could be `repeat()`
