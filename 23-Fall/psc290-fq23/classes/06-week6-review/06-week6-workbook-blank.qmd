...
soep_long |>
  filter(!category == "big5") |>
  full_join(codebook |> 
              select(category, label, item_name, year, item_text)
            ) |> 
  arrange(category)
```
...
```{r}
soep_big5 <- soep_long |>
  dplyr::filter(category == "big5") |> 
  dplyr::mutate(value = ifelse(value < 1, NA, value)) |> 
  tidyr::drop_na() |> 
  dplyr::with_groups(c(Proc_SID, label, year),
                     summarise,
                     value = mean(value))
soep_big5
```
...
```{r}
soep_le <- soep_long |>
  dplyr::filter(category == "le") |> 
  dplyr::mutate(
    value = mapvalues(value, seq(-8, 1), c(rep(NA, 6), 0, NA, NA, 1))
...
soep_nested <- soep_clean |>
  dplyr::group_by(trait, le) |> 
  tidyr::nest(-c(trait, le)) |> 
  dplyr::ungroup()
```
...
soep_nested <- soep_nested |>
  dplyr::mutate(model = purrr::map(data, lmer_fun))
soep_nested
```
...
```{r}
soep_nested <- soep_nested |>
  dplyr::mutate(npeole = map_dbl(model, nslopes_fun))
soep_nested
```
