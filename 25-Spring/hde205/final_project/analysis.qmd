...
preds[3] |> 
  purrr::map(
    ~lmerTest::lmer(
      igi_escore ~ poly(time, 2) * randomizacao * . + (1 | record_id), 
      REML = FALSE, data = mydata3
      )
    )

# Now, fit the LMM with the three-way interaction
# This is a complex model, start with a random intercept and build up if possible
final_lmm <- lmerTest::lmer(
  igi_escore ~ poly(time, 2) * randomizacao * spaq_score_t1 +
               (1 | record_id),
  REML = FALSE,
  data = mydata3
)
