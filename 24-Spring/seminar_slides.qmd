...
- Their experiences and level of mastery of different topics are almost certainly different.
...
- A new educational program is shown to increase mean achievement

- However, it also increases achievement dispersion such that the number of students failing to meet minimum level of achievement increases.
...
- To identify clustering units (students, classrooms, etc.) that exhibit either unusually large or unusually small within-cluster variance.
...
that is captured in the conditional means.

## MELSM

- Does not model the residuals themselves but, instead, the error \textit{variance} $\boldsymbol{\varphi}$ from which the residuals are realized.

- It *simultaneously* estimate the means and the residual variance.
...
- The subscript to the indicator ($\delta_i$) assigns each unit a prior inclusion probability.

- When a 0 is sampled, the portion after the fixed effect drops out of the equation.

- For that particular MCMC sample, their estimate will then be equivalent to the average ($\alpha_{0j} =   \eta_{00}$)--i.e., 
...
\eta_{00}, & \text{if }\delta_i = 0 , \\
\eta_{00} + t_{0j}, & \text{if }\delta_i = 1
\end{cases}.
\end{equation}
:::
...
\begin{align}
BF = \frac{Pr(\alpha_{0j} = \eta_{00} | \textbf{Y})}{1- Pr(\alpha_{0j} = \eta_{00} | \textbf{Y})}
\end{align}
...
- The fitted model used four chains of 1,000 interations after a burn-in period of 1,000 iterations which resulted in a total of 2,000 samples from the posterior distribution.
