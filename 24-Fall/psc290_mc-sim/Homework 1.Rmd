...
\textcolor{red}{The required sample size is `r n` according to the analytical solution. By simulating `r reps` random t-values from a t-distribution with `r n- 1` degrees of freedom, we observe that `r prop_tails *100`\% of the values are larger than the critical z-value of `r upper_critical`. The monte carlo error was found to be `r mc_error`.}